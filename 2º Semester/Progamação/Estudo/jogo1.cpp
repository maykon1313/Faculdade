...
void sprint_enemie(int tipo) {
    if (tipo == 1) printf("1 ");  // Inimigo 1
    if (tipo == 2) printf("2 ");  // Inimigo 2
    if (tipo == 3) printf("3 ");  // Inimigo 3
}

void sprint_caminhos(int parede) {
    if (parede == 4) printf("O ");  // Caminho livre
    if (parede == 5) printf("@ ");  // Parede
    if (parede == 6) printf("X "); // Inimigo morto
}

void sprint_central(int num, int direcao) {
    if (num == 1) sprint_tanque(direcao);
    if (num < 4) sprint_inimigos(num);
    else sprint_caminhos(num);
}


// map
void show_map(int map[10][10], int direction) {
    for (int i = 0; i < 10; ++i) {
        for (int j = 0; j < 10; ++j) {
            sprint_central(map[i][j], direction);
        }
        printf("\n\n");
    }
}

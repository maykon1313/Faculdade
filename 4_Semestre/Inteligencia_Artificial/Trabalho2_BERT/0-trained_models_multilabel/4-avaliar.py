...
print("Avaliação será salva em: .model/avaliacao.txt")

sys.stdout = open(os.path.join(os.path.dirname(__file__), '..', 'model', 'avaliacao.txt'), 'w', encoding='utf-8')
...
for idx in range(len(all_labels[0])):
    ...
    for cat_idx in range(len(category_names)):
        if all_labels[cat_idx][idx] != all_preds[cat_idx][idx]:
            has_error = True
            break
